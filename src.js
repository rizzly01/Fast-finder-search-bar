...
// Updated dummy data for countries (replace with actual API call in a real application)
const countries = [
  {
  ...
  return (
    <div className="max-w-md mx-auto mt-10">
      <div className="relative">
        <input
          type="text"
          className="w-full px-4 py-2 text-gray-700 bg-white border rounded-full focus:border-blue-400 focus:outline-none focus:ring focus:ring-blue-300 focus:ring-opacity-40"
      ...
      {suggestions.length > 0 && (
        <ul className="mt-2 bg-white border rounded-md shadow-lg">
          {suggestions.map((country, index) => (
            ...
            >
              <span className="font-semibold">{country.country}</span> - {country.capital}
            </li>
          ))}
        </ul>
      )}
      {selectedCountry && (
        <div className="mt-4 p-4 bg-white border rounded-md shadow-lg">
          <h2 className="text-2xl font-bold mb-2">{selectedCountry.country}</h2>
          <p><strong>Capital:</strong> {selectedCountry.capital}</p>
          <p><strong>Population:</strong> {selectedCountry.population.toLocaleString()}</p>
          <p><strong>Official Language:</strong> {selectedCountry.official_language}</p>
